################################################################################
#                  SINGLE PROGRAMME SEARCH (afghanian)
################################################################################
...
tb_fs %>% 
  ...
  count() %>% 
  ungroup() %>% 
  complete(Programme, year = 2018:2022, 
           fill = list(n = 0)) %>% 
  na.omit() %>% 
  group_by(Programme) %>% 
  mutate(cumsum=cumsum(n)) %>% 
  ...
  scale_y_continuous(breaks = seq(1,18,2), limits = c(0,18))
...
tb_fs %>% 
  ...
  scale_x_continuous(breaks = seq(2018,2022,1), limits = c(2018,2022))
...
growth %>% 
  ...
  geom_density_ridges(rel_min_height = 0.01, scale = 1000, alpha = 0.7)+
