...
multi_pg <-
  ...
  filter(Programme == "UKR" | Programme == "HR" | 
           Programme == "BLR")
...
tb_fs %>% 
  ...
  count() %>% 
  ungroup() %>% 
  complete(Programme, year = 2016:2022, 
           fill = list(n = 0)) %>% 
  na.omit() %>% 
  group_by(Programme) %>% 
...
tb_fs %>% 
  ...
  ylab("")+
  scale_fill_viridis(discrete = TRUE) +
  scale_x_continuous(breaks = seq(2016,2023,1), limits = c(2016,2023))
