...
multi_nat <-
  ...
  filter(citizenship.countryDescription == "AFGHANISTAN" | citizenship.countryDescription == "RUSSIAN FEDERATION" | 
           citizenship.countryDescription == "SYRIAN ARAB REPUBLIC")
...
tb_fs %>% 
  ...
  ylab("")+
  scale_fill_viridis(discrete = TRUE) +
  scale_x_continuous(breaks = seq(2006,2022,1), limits = c(2006,2022))
...
growth %>% 
  ...
  geom_density_ridges(rel_min_height = 0.01, scale = 2, alpha = 0.7)+
